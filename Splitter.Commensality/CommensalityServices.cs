    ...
    ISptInterface sptInterface)
{
    ...
    public async Task<SptResult> GetCommensalitysAsync(GetCommensalityCommand command)
    {
        try
        {
            ...
            var specification = new GetByRangeDateEspecification<Models.Commensality>(command.From, command.To, x => x.CreatedAt);
            var result = (IEnumerable<Models.Commensality>)await _commensalityRepository.Filter(specification.IsSatisfiedBy);
    ...
    public async Task<SptResult> CreateCommensality(CreateCommensalityCommand command)
    {
        try
        {
            ...
            var commensality = Models.Commensality.Create(command.CommensalityId, command.Name);
